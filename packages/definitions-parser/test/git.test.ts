...
async function getDeletions(diffs: GitDiff[]): Promise<PackageId[]> {
  ...
  assert.ok(!("error" in changes), "Should not have error property");
...
testo({
  async ok() {
    assert.deepStrictEqual(await getNotNeededPackages(allPackages, await getDeletions(deleteJestDiffs)), jestNotNeeded);
  },
  async gitMovesConvertedToAddsAndDeletes() {
    assert.deepStrictEqual(await gitChanges(moveRdfJSDiffs, createGetAttwJson()), {
      attwChanges: [],
      additions: [
...
// Mock pacote.manifest for the notNeededPackage tests
const mockManifest = mock.fn(async (spec: string, opts?: pacote.Options) => {
  switch (spec) {
...
// Apply the mock
mock.method(pacote, "manifest", mockManifest);
...
testo({
  async missingSource() {
    assert.deepStrictEqual(await checkNotNeededPackage(nonexistentReplacementPackage), [
...
Unneeded packages have to be replaced with a package on npm.`,
  ...
  async missingTypings() {
    assert.deepStrictEqual(await checkNotNeededPackage(nonexistentTypesPackage), [
      "Unexpected error: @types package not found for @types/nonexistent",
    ]);
  },
  async deprecatedSameVersion() {
    assert.deepStrictEqual(await checkNotNeededPackage(sameVersion), [
...
it is supposed to replace, 50.0.0 of @types/jest.`,
  ...
  async deprecatedOlderVersion() {
    assert.deepStrictEqual(await checkNotNeededPackage(olderReplacement), [
      `The specified version 4.0.0 of jest must be newer than the version
it is supposed to replace, 50.0.0 of @types/jest.`,
    ]);
  },
  async missingNpmVersion() {
    assert.deepStrictEqual(await checkNotNeededPackage(nonexistentReplacementVersion), [
      "The specified version 999.0.0 of jest is not on npm.",
    ]);
  },
  async ok() {
    await checkNotNeededPackage(newerReplacement);
  },
});
